...
/** Vista del lado comercio, mismo criterio que MerchantDashboard.tsx: no hay
 * login de comercio en este sandbox, se navega eligiendo el comercio desde
 * un selector. */
export function FactoringView() {
  const merchants = useKoraStore((s) => s.merchants);
  const [merchantId, setMerchantId] = useState(Object.keys(merchants)[0] ?? '');
